...
data<-data[is.na(data$VIQ220)!=TRUE & data$VIQ220!=9,]
...
for (i in 1:9){
  tages[i] <- nrow(data[((data$RIDAGEMN/12)<(i*10)&(data$RIDAGEMN/12)>((i-1)*10)),])
  gccount[i] <- nrow(data[data$VIQ220==1&((data$RIDAGEMN/12)<(i*10)&(data$RIDAGEMN/12)>((i-1)*10)),])
...
Model 1 w/ just age as predictor
```{r}
lm()
```
